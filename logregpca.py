...
def run():
    ...
    generate_models(configuration.dimensions_logregpca, dataset)


def generate_models(correlations, dataset):
    print("generating logistic regression models - with pca")
    ...
    for dep in configuration.dependent:
        ...
        loop_all_combinations_for(dep, correlations, dataset)
...
def loop_all_combinations_for(dep, correlations, dataset):
    ...
    subset = correlations
    for pca_n in range(configuration.pca_min_n, min(configuration.pca_max_n + 1, len(correlations))): 
